...
export class ClientNetwork {
    ...
    getSessionStorageKey(
        ...
        return `site-game:session:${String(
            roomCode
        )
            .trim()
            .toUpperCase()}`;
    ...
    getOrCreateSessionId(
        ...
        try {
            ...
            /*
             * Даже если localStorage
             * недоступен, текущая сессия
             * всё равно сможет работать.
             */
        }
    ...
    async connectSignaling(
        ...
        if (!this.signalingRoom) {
            throw new Error(
                "Room code is required for signaling."
            );
        }
    ...
    handleSignalingMessage(
        ...
        if (
            ...
            if (!connectionId) {
                this.emit(
                    "error",
                    new Error(
                        "HOST sent an offer without a connection ID."
                    )
                );
            ...
            this.connect(
                ...
                .then(
                    answer => {
                        ...
                        if (!sent) {
                            throw new Error(
                                "Signaling connection closed before the WebRTC answer was sent."
                            );
                        }
                    }
                )
                .catch(
                    error => {
                        console.error(
                            "Failed to create automatic WebRTC answer:",
                            error
                        );
        ...
        if (
            ...
            this.emit(
                ...
                new Error(
                    message.message ||
                    "Signaling server could not deliver the message."
                )
            );
    ...
    setupChannel() {
        ...
        this.channel.onopen =
            () => {
                ...
                /*
                 * ВАЖНО:
                 * JOIN отправляется только
                 * после открытия DataChannel.
                 *
                 * sessionId позволяет HOST
                 * понять, что это старый игрок.
                 */
                this.join(
                    this.playerName
                );
            ...
            setTimeout(
                () => {
                    ...
                    this.emit(
                        "offerTimeout",
                        new Error(
                            "HOST did not send a WebRTC offer within 15 seconds."
                        )
                    );
                },
                timeout
    ...
    scheduleReconnect() {
        ...
        if (
            ...
            this.emit(
                "reconnectFailed",
                new Error(
                    "Не удалось восстановить подключение к комнате."
                )
            );
